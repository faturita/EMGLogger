...
/***
  ...
  *		-1 if error
  */
int opiuce_settserftxpwr(HANDLE *comportptr, int rftxpwr)
{
    OPIPKT_t opipkt;
...
{
    pktptr->dataCode = 0x14;
    pktptr->payload[0] = 0x00;
    pktptr->length = 1;
    if(opipkt_put_com(pktptr, comportptr) != 0) return -1;
    if(opipkt_get_com(pktptr, comportptr) != 0) return -1;
    if((pktptr->dataCode == 0x14) && (pktptr->payload[0] == 0x01))
        return 0;
    else return -1;
}
...
/***
  ...
  *		-1 if error
  */
int opiuce_getwltsefft(HANDLE *comportptr, OPIPKT_t *pktptr)
{
    pktptr->dataCode = 0x14;
    pktptr->payload[0] = 0x22;
    pktptr->length = 1;
    if(opipkt_put_com(pktptr, comportptr) != 0) return -1;
    if(opipkt_get_com(pktptr, comportptr) != 0) return -1;
    if((pktptr->dataCode == 0x14) && (pktptr->payload[0] == 0x23))
        return 0;
    else return -1;
}
...
{
    pktptr->dataCode = 0x14;
    pktptr->payload[0] = 0x04;
    pktptr->length = 1;
    if(opipkt_put_com(pktptr, comportptr) != 0) return -1;
    if(opipkt_get_com(pktptr, comportptr) != 0) return -1;
    if((pktptr->dataCode == 0x14) && (pktptr->payload[0] == 0x05))
        return 0;
    else return -1;
}
...
int opiuce_setrelaxparams(HANDLE *comportptr, int thx, int thm, int offl, int th3gm, int th3bm, int th2gm, int th2bm, int th1gm, int th1bm, int offm)
{
    ...
    opipkt.dataCode = 0x14;
    opipkt.payload[0] = 0x06;
    opipkt.length = 21;
    opipkt.payload[1] = thx >> 8;
    opipkt.payload[2] = thx;
    opipkt.payload[3] = thm >> 8;
    opipkt.payload[4] = thm;
    opipkt.payload[5] = offl >> 8;
    opipkt.payload[6] = offl;
    opipkt.payload[7] = th3gm >> 8;
    opipkt.payload[8] = th3gm;
    opipkt.payload[9] = th3bm >> 8;
    opipkt.payload[10] = th3bm;
    opipkt.payload[11] = th2gm >> 8;
    opipkt.payload[12] = th2gm;
    opipkt.payload[13] = th2bm >> 8;
    opipkt.payload[14] = th2bm;
    opipkt.payload[15] = th1gm >> 8;
    opipkt.payload[16] = th1gm;
    opipkt.payload[17] = th1bm >> 8;
    opipkt.payload[18] = th1bm;
    opipkt.payload[19] = offm >> 8;
    opipkt.payload[20] = offm;
